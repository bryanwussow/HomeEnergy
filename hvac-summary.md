...
**Preliminary loop design (2026-08-28):** `geothermal loop design - preliminary.md` runs the design prompt against the above figures plus `Wussow Ground Loop Plot Plan.pdf`. Recommended design: **~1,800 ft total pipe (900 ft per loop, 2 loops)**, 1" SDR-11 HDPE, ~9 GPM system flow, ~20% methanol (default, preferred by contractor; ~20% propylene glycol as alternate). Cooling load governs sizing (not heating). Several inputs remain unconfirmed pending soil testing and contractor/PE review — see that document's "Assumptions and Flags."
...
| 2026-08-28 | Swapped the preliminary loop design's default fluid: ~20% methanol is now the default (preferred by the contractor), with ~20% propylene glycol as the alternate — was the reverse. Also: added a contractor-facing `Geothermal Loop - Contractor Scope Brief.md`, marked trench-layout Option B (wide separation) as the homeowner's preferred approach, and split the manifold-location constraint into two open options (inside basement vs. below-grade vault) for contractor input — see `geothermal loop design - preliminary.md` and `geothermal-loop-design-prompt.md` for full detail | "geothermal loop design" session, continued |
