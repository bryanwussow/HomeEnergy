...
Both Option A (single narrow trench) and Option B (wide separation) fit comfortably in the available loop field either way — the choice is a cost/performance trade-off, not a space constraint.
...
- k (soil conductivity) = 0.9 Btu/(hr·ft·°F) — moist clay, per Project Parameters (**not soil-tested; flagged**)
- Pipe: 1" nominal SDR-11 HDPE, OD d = 1.315 in = 0.1096 ft
- Depth z = 5 ft (trench depth, per constraint #1)
- 2z/d = 10/0.1096 = 91.3 → cosh⁻¹(91.3) ≈ 5.21 → S' = 2π/5.21 = **1.21 ft⁻¹**
- q' = 0.9 × 1.21 × ΔT = **1.09 × ΔT** Btu/(h·ft)

**Step 3 — Design ΔT (ground temp vs. design fluid temp)**, using the ground temps from Project Parameters and assumed design entering-water-temperatures (EWT) typical for a mild-climate horizontal loop with antifreeze (**flagged: confirm min/max EWT against the actual WaterFurnace 5-Series spec sheet**):
...
- **Near the house:** both loops converge; outgoing lines run at 24 in. depth, return lines in a separate sleeve 12 in. below (≈36 in. depth), optionally separated by 2 in. rigid foam, before entering the walkout basement wall as continuous joint-free pipe.
- **Manifold:** located inside the walkout basement (not an exterior vault) — all fusion joints, valves, and the supply/return manifold live inside, accessible and free of below-grade joints.
...
- **Fluid:** ~20% propylene glycol — gives freeze protection well below the ~30°F design min EWT, without the flammability/handling concerns of methanol. Confirm against manufacturer-approved fluid list.
...
- **Pressure testing:** standard IGSHPA practice — pressurize to ~100 psi, hold and monitor (e.g., 30 min–24 hr) with no pressure loss, before backfilling any section.
- **Wall penetration:** sealed sleeve through the basement foundation wall (hydraulic cement or a purpose-made geothermal wall boot) to prevent water intrusion, since the manifold sits inside per constraint #4.
...
- **Assumptions/flags requiring field verification or PE/contractor sign-off before construction:** soil conductivity (0.9 Btu/hr·ft·°F, untested), ground temperatures (regional estimate + canopy correction, not measured), design min/max EWT (30°F/90°F, assumed), COP/EER figures (borrowed from the 2.5-ton unit spec for this 3-ton target), flow rate (3 GPM/ton assumed), groundwater table depth (assumed dry above 6 ft), and final setbacks (pending county permit).
