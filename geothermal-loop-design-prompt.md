...
2. **Pipe convergence at house:** Near the house entrance, where the loop(s) converge, the outgoing line(s) run at 24 in. below the surface and enter the house through their own wall penetration; the return line(s) run in a separate sleeve, offset 12 in. below the outgoing line(s) (i.e., ~36 in. depth), through their own separate, nearby wall penetration — keeping this vertical separation through the wall itself, rather than converging to one shared opening, to avoid thermal short-circuiting between them. A 2 in. rigid foam (e.g., XPS) layer can be placed between the outgoing and return lines in this section for further protection against short-circuiting. The two penetrations should be close enough together vertically that a single work area inside can access and manifold both lines — one entry point and one exit point at the house, not one shared opening.
