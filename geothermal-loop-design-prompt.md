...
- Heat pump capacity: 2.5-ton / 30,000 Btu/h nominal, WaterFurnace 5 Series ground-source heat pump (options W1/W2 in `hvac-summary.md`), single/multi-speed per manufacturer spec — confirm exact model once selected
- Building heating load / cooling load: Manual J (Main Floor only, the zone the heat pump serves) — **20,597 Btu/h heating, 17,243 Btu/h cooling** (14,582 sensible + 2,661 latent). Equipment is being sized at 2.5 tons (30,000 Btu/h), above the Manual J heating load — this matches the existing system's field-validated performance (years of real-world operation confirm 2.5 tons is adequate for both heating, with wood-stove/heat-strip assist, and cooling; see `hvac-summary.md` §2 and the `heat_pump_sizing_field_validation` memory). Size the loop to the 2.5-ton/30,000 Btu/h nameplate capacity, not the raw Manual J number.
